...
function validateWebhookData(data: Record<string, string>): {
  ...
  const validStatuses = ['queued', 'sent', 'delivered', 'failed', 'undelivered', 'receiving', 'received']
...
webhooks.post('/sms-status', async (c) => {
  try {
    ...
    const url = c.req.url
    const isValidSignature = await validateTwilioSignature(signature, url, body)
