...
export class SMSService {
  ...
  async sendSMS(message: SMSMessage): Promise<SMSResult> {
    try {
      ...
      const rateLimitResult = await this.rateLimiter.checkRateLimit(message.userId)
      
      if (!rateLimitResult.allowed) {
        logger.warn(`SMS rate limit exceeded for user ${message.userId}:`, rateLimitResult.reason)
        return {
          success: false,
   ...
   */
  private async sendWithRetry(message: SMSMessage, attempt: number = 0): Promise<SMSResult> {
    try {
  ...
  private async makeTwilioRequest(
    ...
    const url = `https://api.twilio.com/2010-04-01/Accounts/${config.twilio.accountSid}/${endpoint}.json`
  ...
  async getRateLimitStatus(userId: string): Promise<{
    ...
    try {
      ...
      return {
        hourlyRemaining: Math.max(0, config.smsLimits.maxSMSPerUserPerHour - usage.hourlyCount),
        dailyRemaining: Math.max(0, config.smsLimits.maxSMSPerUserPerDay - usage.dailyCount),
