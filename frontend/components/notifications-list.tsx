...
export function NotificationsList({ notifications }: NotificationsListProps) {
  ...
  const getChannelIcon = (channel: string) => {
    switch (channel) {
      case "email":
        return <Mail className="h-4 w-4 text-orange-600" data-testid="mail-icon" />
      case "sms":
        return <Phone className="h-4 w-4 text-orange-600" data-testid="phone-icon" />
      default:
        return null
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-orange-700">Filter by channel:</span>
