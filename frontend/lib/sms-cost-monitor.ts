...
export class SMSCostMonitor {
  ...
  async checkUserCostAlert(userId: string): Promise<CostAlert | null> {
    try {
      ...
      const limit = this.config.smsLimits.maxMonthlyCostUSD;
      ...
      console.error("Failed to check user cost alert:", error);
      return null;
    }
  }
  ...
  async getSystemCostStats(): Promise<SystemCostStats | null> {
    try {
      ...
      const totalMonthlyCostUSD = data.reduce(
        (sum, row) => sum + (row.monthly_cost_usd || 0),
        0,
      );
      const totalMonthlyMessages = data.reduce(
        (sum, row) => sum + (row.monthly_count || 0),
        0,
      );
      const activeUsers = data.length;
      const averageCostPerUser = activeUsers > 0
      ...
      console.error("Failed to get system cost stats:", error);
      return null;
    }
  }
  ...
  async getUsersExceedingLimits(): Promise<CostAlert[]> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .gte(
          "monthly_cost_usd",
          this.config.smsLimits.maxMonthlyCostUSD * 0.75,
        ); // 75% threshold
      ...
      console.error("Failed to get users exceeding limits:", error);
      return [];
    }
  }
  ...
  async logCostAlert(alert: CostAlert): Promise<void> {
    try {
      ...
      // like DataDog, New Relic, or send an email/Slack notification
      if (this.config.isProduction && alert.alertLevel === "exceeded") {
        // TODO: Implement production alerting (email, Slack, etc.)
        console.error("CRITICAL: User exceeded SMS cost limit:", alert);
      }
    } catch (error) {
      console.error("Failed to log cost alert:", error);
    }
  }
  ...
  async resetMonthlyCosts(): Promise<{ resetCount: number; error?: string }> {
    try {
      ...
      console.log(`Reset monthly SMS costs for ${resetCount} users`);

      return { resetCount };
    } catch (error) {
      console.error("Failed to reset monthly costs:", error);
      return {
        resetCount: 0,
  ...
  > {
    try {
      ...
      const isOnTrackToExceedLimit =
        projectedMonthlyCost > this.config.smsLimits.maxMonthlyCostUSD;
