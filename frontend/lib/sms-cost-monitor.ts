...
export class SMSCostMonitor {
  ...
  async getUsersExceedingLimits(): Promise<CostAlert[]> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .gte('monthly_cost_usd', this.config.smsLimits.maxMonthlyCostUSD * 0.75) // 75% threshold
  ...
  async getCostProjection(userId: string): Promise<{
    ...
    try {
      ...
      const isOnTrackToExceedLimit = projectedMonthlyCost > this.config.smsLimits.maxMonthlyCostUSD
