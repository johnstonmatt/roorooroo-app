...
export class SMSRateLimiter {
  ...
  async checkRateLimit(userId: string): Promise<RateLimitResult> {
    try {
      ...
      // Check hourly limit
      if (
        updatedUsage.hourlyCount >= this.config.smsLimits.maxSMSPerUserPerHour
      ) {
        return {
          allowed: false,
          reason:
            `Hourly SMS limit exceeded (${this.config.smsLimits.maxSMSPerUserPerHour} messages/hour)`,
          remainingHourly: 0,
        };
      }
      ...
      // Check daily limit
      if (
        updatedUsage.dailyCount >= this.config.smsLimits.maxSMSPerUserPerDay
      ) {
        return {
          allowed: false,
          reason:
            `Daily SMS limit exceeded (${this.config.smsLimits.maxSMSPerUserPerDay} messages/day)`,
          remainingDaily: 0,
        };
      }
      ...
      const projectedCost = updatedUsage.monthlyCostUSD +
        this.config.smsLimits.costPerSMSUSD;
      if (projectedCost > this.config.smsLimits.maxMonthlyCostUSD) {
        return {
          allowed: false,
          reason:
            `Monthly SMS cost limit would be exceeded ($${this.config.smsLimits.maxMonthlyCostUSD})`,
      ...
      return {
        allowed: true,
        remainingHourly: this.config.smsLimits.maxSMSPerUserPerHour -
          updatedUsage.hourlyCount,
        remainingDaily: this.config.smsLimits.maxSMSPerUserPerDay -
