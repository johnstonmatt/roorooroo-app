...
export class SMSService {
  ...
  async sendSMS(message: SMSMessage): Promise<SMSResult> {
    try {
      ...
      if (!rateLimitResult.allowed) {
        console.warn(`SMS rate limit exceeded for user ${message.userId}:`, rateLimitResult.reason);
        return {
          success: false,
  ...
  async getRateLimitStatus(userId: string): Promise<{
    ...
    try {
      ...
      return {
        hourlyRemaining: Math.max(0, this.config.smsLimits.maxSMSPerUserPerHour - usage.hourlyCount),
        dailyRemaining: Math.max(0, this.config.smsLimits.maxSMSPerUserPerDay - usage.dailyCount),
