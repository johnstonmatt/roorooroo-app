...
.stackblitz/
  codeflow.json     - StackBlitz pr.new bot config

scripts/
  setup-twilio.sh   - Twilio credential wizard (deno task setup-twilio)
  setup-branching.sh - Branching + pr.new wizard (deno task setup-branching)

.github/workflows/
  fns-push.yml      - CI/CD for Edge Function deployment
  preview-env.yml   - Comments the pr.new link + preview branch env on PRs
```
...
Every PR gets two linked pieces:

1. **Supabase preview branch** — created by the Supabase GitHub integration. It
   is a separate Postgres instance that applies `supabase/migrations/` and
   `supabase/seed.sql`, and starts with **no** production data. Destroyed when
   the PR closes.
2. **StackBlitz pr.new workspace** — the CodeflowApp bot comments a one-click
   link that boots the repo in a browser IDE. Configured by
   `.stackblitz/codeflow.json`.

`.github/workflows/preview-env.yml` bridges the two: it waits for the preview
branch, reads its API URL and anon key, and posts a sticky PR comment with the
pr.new link plus the env vars to paste in. pr.new runs in the browser and cannot
read repository secrets, which is why the values have to be surfaced in the
comment.

First-time setup is dashboard work — run `deno task setup-branching`, which
walks through the GitHub integration, enabling branching, and installing the
CodeflowApp bot, then verifies the result.
