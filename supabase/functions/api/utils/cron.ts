 ...
 * @returns Promise<CronJobResult> - Result with success status and details
 */
export async function updateMonitorCronJob(
  config: CronJobConfig,
): Promise<CronJobResult> {
  try {
    // Validate configuration
    ...
    if (!exists) {
      return {
        ...
        details:
          `Job ${jobName} does not exist for monitor ${config.monitorId}`,
      };
    }
 ...
 */
export async function getCronJobInfo(
  monitorId: string,
 ...
 */
export async function batchCreateCronJobs(
 ...
 */
export async function batchDeleteCronJobs(
  monitorIds: string[],
): Promise<CronJobResult[]> {
  const results: CronJobResult[] = [];
 ...
 */
export async function cleanupOrphanedCronJobs(
  userId: string,
): Promise<CronJobResult> {
  try {
    const supabase = createServiceClient();
    ...
    for (const jobName of userJobs) {
      const monitorId = jobName.replace("monitor_check_", "").replace(
        /_/g,
        "-",
      );
    ...
    return {
      success: true,
      details:
        `Cleaned up ${deletedCount} orphaned cron jobs out of ${orphanedJobs.length} found`,
    };
  } catch (error) {
...
export function describeCronExpression(cronExpression: string): string {
  ...
  if (
    minute.startsWith("*/") && hour === "*" && day === "*" && month === "*" &&
    weekday === "*"
  ) {
  ...
  if (
    minute === "0" && hour.startsWith("*/") && day === "*" && month === "*" &&
    weekday === "*"
  ) {
