 ...
 * Requires authentication middleware
 */
monitors.put('/:id', asyncHandler(async (c) => {
  const userId = c.get('userId')
  const supabase = c.get('supabase')
  ...
  try {
    ...
    const updateSchema = {
      ...
      pattern: { required: false, type: 'string' as const, minLength: 1, maxLength: 500 },
      pattern_type: { 
        required: false, 
        type: 'string' as const, 
        enum: ['contains', 'not_contains', 'regex'] 
    ...
    if (body.pattern_type !== undefined) updateData.pattern_type = body.pattern_type
    if (body.check_interval !== undefined) updateData.check_interval = body.check_interval
 ...
 * Requires authentication middleware
 */
monitors.delete('/:id', asyncHandler(async (c) => {
  const userId = c.get('userId')
  const supabase = c.get('supabase')
  const monitorId = c.req.param('id')
