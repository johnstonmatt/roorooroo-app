 ...
 */
admin.get('/sms-costs', asyncHandler(async (c) => {
  try {
 ...
 */
admin.post('/sms-costs', asyncHandler(async (c) => {
  try {
    ...
    if (!action || typeof action !== 'string') {
      return c.json({
        ...
        validActions: ['reset-monthly-costs', 'check-user-alert', 'get-user-projection']
      }, 400)
    }
    ...
    if (['check-user-alert', 'get-user-projection'].includes(action)) {
      if (!userId || typeof userId !== 'string') {
        return c.json({
          error: `Missing or invalid required field: userId for ${action} action`
        }, 400)
      }
    ...
    switch (action) {
      ...
      case 'check-user-alert': {
        if (!userId) {
          return c.json({
            error: 'Missing required field: userId for check-user-alert action'
          }, 400)
        }
      ...
      case 'get-user-projection': {
        if (!userId) {
          return c.json({
            error: 'Missing required field: userId for get-user-projection action'
          }, 400)
        }
