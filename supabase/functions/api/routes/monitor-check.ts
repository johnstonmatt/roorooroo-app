 ...
 */
monitorCheck.post('/', asyncHandler(async (c) => {
  ...
  try {
    ...
    if (checkResult.status !== monitor.last_status && monitor.notification_channels?.length > 0) {
      ...
      } else if (checkResult.status === 'found' && monitor.last_status !== 'found') {
        notificationType = 'pattern_found'
      } else if (checkResult.status === 'not_found' && monitor.last_status === 'found') {
        notificationType = 'pattern_lost'
      } else {
        // No notification needed for this status change
        notificationType = checkResult.status === 'found' ? 'pattern_found' : 'pattern_lost'
      }
...
async function performMonitorCheck(monitor: Monitor): Promise<{
  ...
  try {
    ...
    const patternResult = checkPattern(content, monitor.pattern, monitor.pattern_type)
...
function checkPattern(content: string, pattern: string, patternType: string): {
  ...
  try {
    switch (patternType) {
      case 'contains':
        const containsMatch = content.toLowerCase().includes(pattern.toLowerCase())
        if (containsMatch) {
      ...
      case 'not_contains':
        const notContainsMatch = !content.toLowerCase().includes(pattern.toLowerCase())
